...
\NewDocumentCommand\dummy{}{\setlength{\fboxsep}{0pt}\fbox{\phantom{X}}}

\NewDocumentCommand{\makerow}{m O{}}{\pgfkeys{/variants, default, #2}%
	%
	#1
	%
	& $\dummy_{#1}$              & \if-\rowSUB\else$\dummy\rowSUB$\fi
	& $\dummy^{#1}$              & \if-\rowSUP\else$\dummy\rowSUP$\fi
	& \dummy\textsubscript{#1}   & \if-\rowSUB\else\dummy\rowSUB\fi
	& \dummy\textsuperscript{#1} & \if-\rowSUP\else\dummy\rowSUP\fi
	%
	\\%
}
...
\begin{document}
\begin{tabular}{c|ll ll|ll ll}  % chktex 44
\toprule
	\multicolumn{1}{c|}{}
