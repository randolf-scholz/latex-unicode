...
\def\0{\setlength{\fboxsep}{0pt}\fbox{\phantom{X}}}

\NewDocumentCommand{\makerow}{m O{}}{%
\pgfkeys{/makerow, default, #2}% set defaults
	#1%
	% math mode
	& $\0_{#1}$              & \if-\rowSUB\else$\0\rowSUB$\fi
	& $\0^{#1}$              & \if-\rowSUP\else$\0\rowSUP$\fi
	& \0\textsubscript{#1}   & \if-\rowSUB\else\0\rowSUB\fi
	& \0\textsuperscript{#1} & \if-\rowSUP\else\0\rowSUP\fi
	% end
	\\%
}
