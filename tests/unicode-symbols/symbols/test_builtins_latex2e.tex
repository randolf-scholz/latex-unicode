...
\NewDocumentCommand{\makerow}{m O{}}{\pgfkeys{/variants, default, #2}%
	%
	#1 & \umeaning{#1} & \rowTEXT & #1 & \rowMATH & $#1$ \\%
}

\NewDocumentCommand{\maketext}{m O{}}{\pgfkeys{/variants, default, #2}%
	%
	#1 & \umeaning{#1} & \rowTEXT & #1 && \\%
}

\NewDocumentCommand{\makemath}{m O{}}{\pgfkeys{/variants, default, #2}%
	%
	$#1$ & \umeaning{#1} &&& \rowMATH & $#1$ \\%
}

\begin{document}
\begin{tabular}{cl|cc|cc}  % chktex 44
\toprule
